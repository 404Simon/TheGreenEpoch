...
region whose grid carbon intensity $carbon(t)$ is given as a time series at
5-minute resolution.
...
$E(t) \cdot carbon(t)$.  Total wall-clock time includes both active training time
...
\begin{itemize}
  \item $\theta_p$ (gCO$_2$eq/kWh): pause threshold.  When training and
    $carbon(t) > \theta_p$, training pauses.
  \item $\theta_r$ (gCO$_2$eq/kWh): resume threshold.  When paused and
    $carbon(t) \le \theta_r$, training resumes.
...
\end{align*}
where $GHG_{b}, T_b$ are baseline (greenhouse gas) emissions and time, and $GHG_{p}, T_p$ are policy
emissions and time.
...
\]
with feasibility constraint $O \le B$.  The
...
use $\alpha = 1$, which simplifies the score to $\frac{S}{200}$---pure savings maximization subject to
the overhead budget.
