...
\begin{frame}{Verification}{2.3 Progress and Runtime Consistency}
  ...
  \[
    \Delta t_{\text{checkpoint}} =
    \begin{cases}
      \tau_{\text{pause}} & \text{if a pause transition begins at step } k \\
      \tau_{\text{resume}} & \text{if a resume transition begins at step } k \\
      0 & \text{otherwise}
    \end{cases}
  \]
\end{frame}
  ...
  \begin{enumerate}
    ...
    \item Compute the expected policy runtime
      \[
        ...
        \mathit{pause\_count} \cdot  (\tau_{\text{pause}} + \tau_{\text{resume}}). % \mathit{checkpoint\_overhead\_time}.
      \]
