...
model architectures.  For models run under the same region and carbon-aware
policy, baseline emissions should approximately scale with total data center
energy, driven by GPU count, power, GPU-hours, and power usage effectiveness.  Similarly, absolute
savings should scale with baseline emissions.  Ratios of simulated emissions
and savings between models are compared against ratios implied by their
GPU-hours and power.  Qualitatively implausible outcomes---such as a smaller
model emitting far more than a larger one under identical assumptions---would
indicate issues.
...
but also increase the number of pauses and associated time and energy overhead.
The observed runtime extension and additional energy consumption are compared
to what is implied by the configured per-checkpoint overhead and the recorded
pause count.  The resulting savings-overhead trade-off is checked for
qualitatively reasonable behaviour and diminishing returns at very fine
granularity.
...
\subsection{Optimizer Verification}

We additionally verify the adaptive grid-search optimizer by comparing its
output against brute-force enumeration on a downsampled problem.  For a
reduced grid ($N=20$, one region, one model), we enumerate all 400
(threshold, hysteresis) combinations and confirm that the optimizer's best
point matches the global optimum found by exhaustive search.  The optimizer
converges to within 1\% of the global optimum across all tested configurations.
